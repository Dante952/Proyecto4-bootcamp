...
public class CustomerServiceImpl implements CustomerService {
    ...
    public Mono<Customer> update(final String dni, Customer customer) {
        ...
        return findByRuc(dni).switchIfEmpty(Mono.empty()).filter(Objects::nonNull).flatMap(dbCustomer -> {
            dbCustomer.setBusinnesName(customer.getBusinnesName());
    ...
    @Override
    public Mono<Customer> delete(final String ruc) {
        return findByRuc(ruc).switchIfEmpty(Mono.empty()).filter(Objects::nonNull).flatMap(customer -> {
    ...
    public Mono<Customer> findByRuc(String ruc) {
        return this.customerRepository.findFirstByRuc(ruc);
    }
